...
public class TomlParserTest extends TomlMapperTestBase {
    ...
    static ObjectNode json(String json) throws JacksonException {
        return (ObjectNode) JSON_MAPPER.readTree(json);
    }

    static ObjectNode toml(String toml) throws JacksonException {
    ...
    public void floats() throws Exception {
        ...
        if (!json.equals(toml)) {
            for (Entry<String, JsonNode> entry : json.properties()) {
                ...
                assertEquals(jsonValue, tomlValue,
                        String.format("Entry '%s' (JSON %s vs TOML %s) differs",
                        key, jsonValue.getClass().getSimpleName(), tomlValue.getClass().getSimpleName())
                        );
            }
            // Should not happen but...
            fail("Internal error: could not find difference in JSON vs TOML nodes");
        }
    }
